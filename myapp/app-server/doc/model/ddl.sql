...
-- 강의실 사진
DROP TABLE IF EXISTS lms_claas_photo RESTRICT;
...
CREATE TABLE lms_teacher (
  member_id INTEGER NOT NULL COMMENT '강사번호', -- 강사번호
...
-- 강의실 사진
CREATE TABLE lms_claas_photo (
  class_phto_id     INTEGER      NOT NULL COMMENT '강의실사진번호', -- 강의실사진번호
  class_id          INTEGER      NOT NULL COMMENT '강의실번호', -- 강의실번호
  photo_path        VARCHAR(255) NOT NULL COMMENT '사진', -- 사진
  mimetype          VARCHAR(50)  NOT NULL COMMENT 'MIMETYPE', -- MIMETYPE
  original_filename VARCHAR(50)  NOT NULL COMMENT '원래사진파일' -- 원래사진파일
)
COMMENT '강의실 사진';

-- 강의실 사진
ALTER TABLE lms_claas_photo
  ADD CONSTRAINT PK_lms_claas_photo -- 강의실 사진 기본키
  PRIMARY KEY (
  class_phto_id -- 강의실사진번호
  );
...
CREATE TABLE lms_application (
  ...
  created_dt           DATE        NOT NULL DEFAULT now() COMMENT '신청일', -- 신청일
...
CREATE TABLE lms_application_state (
  ...
  state_name           VARCHAR(50) NULL     COMMENT '상태명' -- 상태명
)
COMMENT '신청상태';
...
-- 강의실 사진
ALTER TABLE lms_claas_photo
  ADD CONSTRAINT FK_lms_class_TO_lms_claas_photo -- 강의실 -> 강의실 사진
  FOREIGN KEY (
  class_id -- 강의실번호
  )
  REFERENCES lms_class ( -- 강의실
