...
class A {
  ...
  void m2() {
    //non-static 메서드는 A의 인스턴스를 저장하는 this 라는 변수가 있다.
  }
  class X { // inner class
...
public class Exam0110 {
  ...
  public static void main(String[] args) {
    ...
    // 인스턴스 생성
    obj2 = new A.Y();//스테틱 중청 클래스는 바깥 클래스의 인스턴스가 없어도 생성할 수 있다.
    //    obj = new A.X(); // 컴파일 오류! 바깥 클래스의 인스턴스 주소 없이 생성 불가!

    A.m1(); //스테틱 멤버를 사용할 때는 인스턴스가 없어도 된다.
    //A.m2(); // 인스턴스 멤버를 사용하려면 반드시 해당객체가 있어야한다.
