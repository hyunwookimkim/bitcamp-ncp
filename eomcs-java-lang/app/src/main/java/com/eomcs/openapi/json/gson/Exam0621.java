  ...
  public static void main(String[] args) {
    ...
    class MyJsonDeserializer implements JsonDeserializer<Member> {
      ...
      public Member deserialize(JsonElement json, Type typeOfT,
        ...
        //getAsJsonObject 객체에서 리턴 받는다.
        JsonObject jsonObject = json.getAsJsonObject();
        //JsonObject position이라는 객체값이 저장되어 있냐?
        if (jsonObject.get("position") != null) {
          //jsonObject을 Manager객체에 넣어서
    ...
    // 공장에 객체 생성기(JsonDeserializer)를 꼽는다.//MyJsonDeserializer 는 어뎁터(일종의 분석기)
    gsonBuilder.registerTypeAdapter(Member.class, new MyJsonDeserializer());

    // 공장을 통해 Gson 객체를 준비한다.gsonBuilder를 통해서 객체를 준비하면  MyJsonDeserializer가 판단해서  gson에게 커스터마이즈 해서 리턴해준다.
