...
public class BoardDao {
  ...
  public List<Board> findAll() throws Exception {
    try (Connection con = DriverManager.getConnection(
      ...
      //예)("board_id")를 int값으로 빼서 setNo라는 프로퍼티에 담고 제네릭값의 <보드객체>를 담고
      // 그리고 반복문을 돌려서 그것을 리스트에 담는다.
      ArrayList<Board> list = new ArrayList<>();
      while (rs.next()) {
        Board board = new Board();// dto ,vo, domain
  ...
  public Board findBy(String no) throws Exception {
    try (Connection con = DriverManager.getConnection(
        "jdbc:mariadb://localhost:3306/studydb?user=study&password=1111");
        Statement stmt = con.createStatement();
        ResultSet rs = stmt.executeQuery("select * from x_board where board_id = " + no)) {// 번호로 한개만 가져온다.
